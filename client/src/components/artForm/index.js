...
const ArtForm = () => {
  ...
  const [addArt, { error }] = useMutation(ADD_ART, {
    update(cache, { data: { addArt } }) {
      const { artwork } = cache.readQuery({ query: QUERY_ARTWORKS });

      cache.writeQuery({
        query: QUERY_ARTWORKS,
        data: { artwork: [addArt, ...artwork] },
      });
    },
  });
  ...
  return (
    <div>
      ...
      <form onSubmit={handleFormSubmit}>
        ...
        <div>
          <select name="artStatus" id="artStatus" onChange={handleChange}>
            <option selected value="WIP">
              WIP
            </option>
